...
import com.sideproject.travel.repo.language
    ...
    private fun observer() {
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                ...
                mainViewModel.queryViews().collectLatest {
    ...
    private fun dialog() {
        Dialog(requireContext()).also { dialog ->
            ...
            dialog.findViewById<RecyclerView>(R.id.rv_language).also {
                it.adapter = DialogAdapter(Data.languages).apply {
                    setOnItemClickLister { l, _ ->
                        language = l.value
                        viewAdapter.refresh()
                        dialog.dismiss()
                    }
